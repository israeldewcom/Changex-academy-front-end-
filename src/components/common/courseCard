    ...
    <div className="group bg-[#14143A] rounded-xl border border-[#252560] overflow-hidden hover:border-[#6C3BFF] transition-all duration-300 hover:shadow-glow">
      <Link to={`/course/${id}`}>
        <div className="relative overflow-hidden h-48">
          <img
            src={thumbnail || 'https://via.placeholder.com/400x200?text=Course+Thumbnail'}
            alt={title}
            className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
          />
          <div className="absolute top-2 right-2">
            <span className="px-2 py-1 bg-black/70 text-xs rounded-md text-white">
              {level || 'Beginner'}
            </span>
          </div>
        </div>
      ...
      <div className="p-4">
        <Link to={`/course/${id}`}>
          <h3 className="text-lg font-bold mb-2 line-clamp-2 hover:text-[#6C3BFF] transition">
            {title}
          </h3>
        </Link>

        <p className="text-sm text-[#A0A0C8] mb-3 line-clamp-2">
          {description}
        </p>
        ...
        <div className="flex items-center justify-between">
          <div className="text-sm text-[#A0A0C8]">
            by <span className="text-white">{instructor?.name || 'Expert'}</span>
          </div>
          <div className="text-lg font-bold">
            {isFree ? (
              <span className="text-[#00FFB2]">FREE</span>
            ) : (
